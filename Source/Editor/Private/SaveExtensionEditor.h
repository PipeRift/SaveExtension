...
public:
	FSaveActorEditorTabBinding()
	{
		...
		BlueprintEditorTabSpawnerHandle = BlueprintEditorModule.OnRegisterTabsForEditor().AddRaw(
			this, &FSaveActorEditorTabBinding::RegisterBlueprintEditorTab);
		BlueprintEditorLayoutExtensionHandle = BlueprintEditorModule.OnRegisterLayoutExtensions().AddRaw(
			this, &FSaveActorEditorTabBinding::RegisterBlueprintEditorLayout);
	}

	void RegisterBlueprintEditorLayout(FLayoutExtender& Extender)
	{
		Extender.ExtendLayout(FBlueprintEditorTabs::MyBlueprintID, ELayoutExtensionPosition::After,
			FTabManager::FTab(FSaveActorEditorSummoner::TabName, ETabState::OpenedTab));
	}

	void RegisterBlueprintEditorTab(
		FWorkflowAllowedTabSet& TabFactories, FName InModeName, TSharedPtr<FBlueprintEditor> BlueprintEditor)
	{
...
#define RegisterDefaultEventChecked(Class, FuncName)            \
	(FKismetEditorUtilities::RegisterAutoGeneratedDefaultEvent( \
		this, Class::StaticClass(), GET_FUNCTION_NAME_CHECKED(Class, FuncName)))

#define RegisterDefaultEvent(Class, FuncName)                   \
	(FKismetEditorUtilities::RegisterAutoGeneratedDefaultEvent( \
		this, Class::StaticClass(), FName(TEXT(#FuncName))))
